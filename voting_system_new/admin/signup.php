...
<body>
	...
	if(!empty($name)&&!empty($last_name)&&!empty($email)&&!empty($cell_no)&&!empty($select)&&!empty($date)&&!empty($username)&&!empty($password)&&!empty($image_name))
		{
	   $qry=mysqli_query($conn,"insert into `signup` (`id`,`name`,`last_name`,`email`,`cell_no`,`select`,`date`,`username`,`password`,`path`)value('','$name','$last_name','$email','$cell_no','$select','$date','$username','$password','$path')");
	...
	<form method="post" enctype="multipart/form-data" onSubmit="return signup()">
		<table border="0" height="400" width="95%" cellspacing="20" > 
            ...
            <tr>
             ...
             <tr>
                <td><h4>Image</h4></td>
                <td colspan="2"><input name="image" type="file" id="password" class="form-control">
                <p id="passitle" style="color:#F5060A;font-size:18px;text-transform:capitalize;margin-top:10px"></p> 
                </td>
            </tr>
            <tr>
